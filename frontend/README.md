...
- **Addresses:** single-line fields use **`AddressAutocompleteInput`** (`src/components/ui/AddressAutocompleteInput.tsx`): after **3+ characters**, debounced suggestions from **Photon / OpenStreetMap** (`src/lib/photonAddressSuggest.ts`, public `photon.komoot.io` — needs browser network access). When **`companies.country_code`** is set (see **Settings → Company info**), Photon is called with **`countrycode=`** so results stay in that country; `/auth/me` exposes **`active_company_country_code`** for customer/estimate forms. Superadmin **Companies** create/edit also has a per-form country selector tied to the same Photon filter. Hint text under address fields (no sample placeholder in the input); **lists and detail views** use **`AddressMapLink`** → **Google Maps** (`src/lib/googleMaps.ts`). DB: **`DB/23_companies_country_code.sql`**. Not Canada Post / AddressComplete.
...
**Lookups:** `/lookups` — static overview; **`/lookups/blinds-types`** (description supports line breaks; list wraps text), **`/lookups/blinds-product-categories`**, **`/lookups/blinds-extra-options/:kindId`** (e.g. `lifting_system`, `cassette_type`), **`/lookups/order-statuses`** — list, search, show inactive, create, edit (**sort order** for chip sequence), deactivate/restore (`lookups.view` / `lookups.edit`). **`/lookups/estimate-statuses`** — same pattern (`GET`/`POST`/`PATCH`); optional JSON **`code`** (`pending` \| `converted` \| `cancelled` or **`null`**) for built-in vs custom; **Estimate statuses** settings table does not show a workflow/slug column. DB: **`builtin_kind`** (migration **`22_status_estimate_builtin_kind.sql`**), plus **`20_`**, **`21_`**. Estimate edit saves **`status_esti_id`**.
