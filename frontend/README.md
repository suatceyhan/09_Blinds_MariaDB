...
**Orders:** `/orders` — **Show deleted** includes inactive (`active` false) rows; deleted rows are styled and omit edit. **New order** and **Edit order** use **`GET /orders/lookup/blinds-order-options`**: blinds grid is a **table** — **one row per blinds type** (checkbox + name), **columns** = Qty, **product category**, **line note**, **line amount** (lifting/cassette are not on this screen; use separate detail workflow). **Total amount** is **read-only**, equal to the **sum of line amounts**; the API recalculates **`orders.total_amount`** from **`blinds_lines`** on create and when lines are patched. **Order note** (full order) plus per-line notes. **Status** from **`GET /orders/lookup/order-statuses`**, taxable base / tax / down payment, agreement date. **Actions**: view, **Edit**, delete/restore. Company rate: **Settings → Company info**.
