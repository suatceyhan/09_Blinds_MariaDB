...
**Lookups:** `/lookups` — static overview; child pages share **`LookupPageLayout`** (icon header, description, search toolbar, table) where applicable. **`/lookups/blinds-types`**, **`/lookups/blinds-product-categories`**, **`/lookups/product-category-availability`** (company×category matrix; API **`/permissions/product-category-matrix`**), **`/lookups/estimate-statuses`** and **`/lookups/order-statuses`** (same screens as before; API still **`/permissions/estimate-status-matrix`** and **`/permissions/order-status-matrix`**). Direct URL **`/lookups/blinds-extra-options/:kindId`** remains for power users; lifting/cassette are not sidebar items (edit options under **Settings → Blinds line matrices**). **Role / User permissions matrix:** Lookups shows the **full submenu tree**; each child row has its own **`.view` / `.edit`** keys (e.g. **`lookups.blinds_types.view`**; status grids use **`settings.estimate_status_matrix.*`** / **`settings.order_status_matrix.*`**). The **Lookups** hub row keeps **`lookups.view` / `lookups.edit`**. API routes accept the **granular** key **or** the legacy broad key.
