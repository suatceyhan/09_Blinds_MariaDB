...
**Lookups:** `/lookups` — static overview; child pages share **`LookupPageLayout`** (icon header, description, search toolbar, table). **`/lookups/blinds-types`** (description supports line breaks; list wraps text), **`/lookups/blinds-product-categories`**, **`/lookups/blinds-extra-options/:kindId`** (e.g. `lifting_system`, `cassette_type`). **`/lookups/order-statuses`** / **`/lookups/estimate-statuses`** — same layout; **superadmin** gets **New status**, **Edit** (modal), **Deactivate / Restore** (ConfirmModal) via **`POST`/`PATCH /permissions/global-*-statuses`**; others see list + **Status** badge only. Estimate **built-in** rows (response **`code`**) show **Built-in** instead of actions. Per-company enablement remains **Permissions → Order / Estimate status matrix**. DB migrations **`27`**, **`28`**. Estimate edit saves **`status_esti_id`**. **Role / User permissions matrix:** Lookups shows the **full submenu tree**; each child row has its own **`.view` / `.edit`** keys (e.g. **`lookups.blinds_types.view`**). The **Lookups** hub row keeps **`lookups.view` / `lookups.edit`**. API routes accept the **granular** key **or** the legacy broad key. DB migration **`29_lookup_subpage_permissions.sql`** backfills granular grants for roles that already had broad Lookups access.
