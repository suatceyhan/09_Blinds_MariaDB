...
type PendingConfirm =
  | { kind: 'deactivate'; optionId: string; display: string }
  | { kind: 'restore'; optionId: string; display: string }
...
export function BlindsExtraOptionsLookupPage() {
  ...
  useEffect(() => {
    if (!me || !kindId.trim()) return
    let cancelled = false
    ;(async () => {
      try {
        const kinds = await getJson<{ id: string; name: string }[]>('/lookups/blinds-extra-option-kinds')
        const k = kinds.find((x) => x.id === kindId.trim())
        if (!cancelled) setKindName(k?.name ?? kindId)
      } catch {
        if (!cancelled) setKindName(kindId)
      }
    })()
    return () => {
      cancelled = true
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setErr(null)
      try {
        const list = await getJson<Row[]>(
          `/lookups/blinds-extra-options/${encodeURIComponent(kindId.trim())}?${listParams}`,
        )
  ...
  async function onCreate(ev: React.FormEvent) {
    ev.preventDefault()
    if (!canEdit || !name.trim() || !kindId.trim()) return
    const so = Number.parseInt(sortOrder, 10)
    setSaving(true)
    setErr(null)
    try {
      await postJson(`/lookups/blinds-extra-options/${encodeURIComponent(kindId.trim())}`, {
        name: name.trim(),
  ...
  async function onEditSave(ev: React.FormEvent) {
    ev.preventDefault()
    if (!editId || !editName.trim() || !kindId.trim()) return
    const so = Number.parseInt(editSortOrder, 10)
    setEditSaving(true)
    setErr(null)
    try {
      await patchJson(
        `/lookups/blinds-extra-options/${encodeURIComponent(kindId.trim())}/${encodeURIComponent(editId)}`,
        {
          name: editName.trim(),
          sort_order: Number.isNaN(so) ? 0 : so,
        },
  ...
  async function executePending() {
    if (!pending || !kindId.trim()) return
    setConfirmPending(true)
    setErr(null)
    try {
      await patchJson(
        `/lookups/blinds-extra-options/${encodeURIComponent(kindId.trim())}/${encodeURIComponent(pending.optionId)}`,
        {
  ...
  if (!kindId.trim()) {
    return <p className="text-sm text-slate-600">Missing kind in the URL.</p>
  }
    ...
    <>
        ...
        description={
          pending == null
            ...
            : pending.kind === 'deactivate'
              ? `${pending.display} will be hidden from new orders. Matrix links for this option are removed for all companies.`
      ...
      {editId ? (
        ...
        >
          ...
          >
            <h2 className="text-sm font-semibold text-slate-900">Edit option</h2>
      ...
      {showCreate ? (
        ...
        >
          <form
            ...
            className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-xl"
          >
            <h2 className="text-sm font-semibold text-slate-900">New option</h2>
            <div className="mt-3 grid gap-3">
              <label className="block text-sm text-slate-700">
                <span className="mb-1 block font-medium">Name</span>
                <input
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        {r.active ? (
                          <button
                            ...
                            title="Deactivate"
                            onClick={() =>
                              setPending({ kind: 'deactivate', optionId: r.id, display: r.name })
                            }
                          ...
                          <button
                            ...
                            title="Restore"
                            onClick={() =>
                              setPending({ kind: 'restore', optionId: r.id, display: r.name })
                            }
