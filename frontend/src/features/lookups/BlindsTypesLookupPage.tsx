...
/** Trim ends only; keep internal line breaks; normalize CRLF to LF. */
  ...
  async function onCreate(ev: React.FormEvent) {
    ...
    try {
      await postJson('/lookups/blinds-types', {
        name: name.trim(),
        aciklama: normalizeDescription(aciklama),
      })
      setName('')
      setAciklama('')
      setShowCreate(false)
      ...
      {showCreate ? (
        ...
        >
          <form
            ...
            className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-xl"
          >
            <h2 className="text-sm font-semibold text-slate-900">New blinds type</h2>
            <div className="mt-3 grid gap-3">
              <label className="block text-sm text-slate-700">
                <span className="mb-1 block font-medium">Name</span>
                <input
