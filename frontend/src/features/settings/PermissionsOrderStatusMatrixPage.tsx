...
export function PermissionsOrderStatusMatrixPage() {
  ...
  async function createGlobal() {
    ...
    setErr(null)
    try {
      await postJson('/permissions/global-order-statuses', { name: n, sort_order: so })
      setNewName('')
      setNewSort('0')
      await load()
    } catch (e) {
  ...
  async function saveEdit() {
    ...
    setErr(null)
    try {
      await patchJson(`/permissions/global-order-statuses/${editId}`, { name, sort_order: so })
      setEditId(null)
      await load()
    } catch (e) {
  ...
  async function applyActiveToggle() {
    ...
    setErr(null)
    try {
      await patchJson(`/permissions/global-order-statuses/${pendingToggle.id}`, { active: pendingToggle.nextActive })
  ...
  const toggleDescription = (() => {
    ...
    return `${pendingToggle.name} will be hidden from new orders and lookups. Existing records keep their label.`
  })()
    ...
    <div className="mx-auto max-w-[100rem] space-y-4 px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-slate-900">Order statuses</h1>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Edit the label and sort order, or deactivate / restore a status.
              </p>
            </div>
            <ShowDeletedToggle
              checked={showDeleted}
              onChange={setShowDeleted}
              id="order-status-matrix-show-deleted"
            />
          </div>
      ...
      {editId ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
            <h2 className="text-sm font-semibold text-slate-900">Edit global order status</h2>
