...
export function PermissionsEstimateStatusMatrixPage() {
  ...
  async function createGlobal() {
    ...
    setErr(null)
    try {
      await postJson('/permissions/global-estimate-statuses', { name: n })
      setNewName('')
      await load()
    } catch (e) {
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-slate-500">
              Day-to-day list views still use <Link className="text-teal-700 underline" to="/lookups/estimate-statuses">Lookups → Estimate statuses</Link> (filtered by this matrix).
            </p>
          </div>
        </div>
        {canEdit ? (
