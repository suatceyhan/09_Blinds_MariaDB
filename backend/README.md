...
- **Orders workflow engine (`DB/blinds-postgresql.sql`, migration 40):** Status geçişleri koddan bağımsızdır (workflow tanımı DB’dedir). UI “Next” aksiyonunu ve gerekli form alanlarını backend’den okur.
  ...
  - **Settings UI (Order):** `GET/PUT /settings/order-workflow` (permission: `settings.order_workflow.view|edit`). Transitions are **soft-deleted** (`workflow_transitions.deleted_at`); `GET` supports **`include_deleted=true`** for the settings screen. `PUT` syncs the **active** list (omitted rows are soft-deleted; at least one active transition required). Şema: **`DB/blinds-postgresql.sql`** (migration **41** permissions, **42** `deleted_at`).
  - **Settings UI (Estimate):** `GET/PUT /settings/estimate-workflow` (permission: `settings.estimate_workflow.view|edit`). Same soft-delete + sync behavior as Order. Default global seed ve izinler: **`DB/blinds-postgresql.sql`** (migration **43**, **44**).
