...
MariaDB URL örneği: `mysql+pymysql://user:pass@localhost:3306/myapp`
Şema:

- Fresh install (önerilen): **`DB/blinds-mariadb.clean.sql`** (PowerShell: `Get-Content ../DB/blinds-mariadb.clean.sql | mysql -h 127.0.0.1 -P 3306 -u root -p myapp`)

SQL yolu seçtiyseniz `.env` içinde `AUTO_CREATE_TABLES=false` yapın; aksi hâlde uygulama açılışta `create_all_tables()` ile aynı tabloları tekrar oluşturmaya çalışabilir.

Not: PostgreSQL RLS/GUC (`tenant_rls_enabled`) MariaDB'de yoktur; tenant izolasyonu uygulama katmanındaki `effective_company_id` filtreleriyle yapılır.

MariaDB SQL notu: bazı “usage guard” sorguları `orders.blinds_lines` JSON dizisini **`JSON_TABLE`** ile tarar (Postgres’teki `jsonb_array_elements` / `->>` yerine). Bu nedenle MariaDB sürümünüzün `JSON_TABLE` desteklediğinden emin olun.
...
- Auth: `POST /auth/login`, `POST /auth/register`, `POST /auth/refresh`, `GET /auth/me`, `POST /auth/logout`, `POST /auth/change_password`, `POST /auth/switch-role`, `POST /auth/set-default-role`, `POST /password_reset/*`, **`POST /auth/switch-company`** (yeni access token; **`active_company_id`**).
- **Çoklu şirket / kiracı:** Normal kullanıcılar yalnızca **seçili aktif şirket** verisini görür: JWT **`active_company_id`** (`get_current_user` içinde **üyelik doğrulanır**; aksi halde **`users.company_id`** kullanılır). İş mantığı **`effective_company_id`** ile filtreler; PostgreSQL’de **`tenant_rls_enabled`** ise oturum **RLS** (`app.tenant_company_id`) ile ek olarak süzülür. **Superadmin** için RLS bypass vardır; liste/detay uçları süperadmin tarafında **bilinçli `company_id` filtresi** ile uyumludur (`resolve_tenant_company_id`).
...
- **Orders:** `GET /orders` (`search`, **`include_deleted`**, **`status_order_id`**; varsayılan yalnızca `active=true`). Not: eski şema/kodda bu alan **`status_orde_id`** idi (typo). List alanları totals + **`downpayment`**, **`final_payment`**, **`installation_scheduled_start_at`**, `balance`, `tax_amount` (Total sütunu için), status label join, **`workflow_next_status_label`**, `active`. **`GET /orders/{id}`** silinmiş (`active=false`) siparişi de döndürür; yanıtta **`final_payment`**, **`payment_entries`**, **`status_order_builtin_kind`**, **`job_edit_locked`**, ayrıca **`expense_total`**, **`profit`** ve **`expense_entries`**. **`job_edit_locked`**: **`PATCH /orders/{id}`**, **`POST /workflow/transition`**, **`record-payment`**, **`DELETE …/payment-entries/{id}`**, **`POST …/line-item-additions`**, **`attachments`** ve **`line-photos`** **400**; **`POST /orders/{id}/expenses`** (ve expense **`DELETE`**) hâlâ izinlidir. **`POST /orders/{id}/record-payment`** waterfall ile job’a dağıtır (UI’de tek satır görünür: `payment_group_id`). **Expenses (profit only):** **`POST /orders/{id}/expenses`** (`orders.edit`) — gövde `{\"amount\":\"<positive decimal>\",\"note\":\"...\",\"spent_at\":\"<optional ISO>\"}`; ödeme/bakiye değişmez. **`DELETE /orders/{id}/expenses/{expense_uuid}`** — satır soft-delete; **`GET /orders/{id}`** içinde listelenir. Profit hesabı: **(subtotal_ex_tax + tax_amount) − expense_total**.
- **Orders workflow engine (`DB/blinds-postgresql.sql`, migration 40):** Status geçişleri koddan bağımsızdır (workflow tanımı DB’dedir). UI “Next” aksiyonunu ve gerekli form alanlarını backend’den okur.
  ...
  - **`GET /workflow/action-types`**: Settings ve liste UI için aksiyon meta verisi. **`ask_form`** için artık üçüncü liste-modal düğmesi (`ui_secondary`) yok — opsiyonel alanlar modal içinde veya **Confirm** ile gönderilir; düzenleme sayfası satırdaki Edit ile açılır.
  ...
  - **Settings UI (Order):** `GET/PUT /settings/order-workflow` (permission: `settings.order_workflow.view|edit`). Transitions are **soft-deleted** (`workflow_transitions.deleted_at`); `GET` supports **`include_deleted=true`** for the settings screen. `PUT` syncs the **active** list (omitted rows are soft-deleted; at least one active transition required). On the **first PUT**, if the company workflow has **no rows yet**, global template transitions are **cloned** into the company definition first (same rationale as Estimate — otherwise a deleted transition never exists under the company def). Şema: **`DB/blinds-postgresql.sql`** (migration **41** permissions, **42** `deleted_at`). Varsayılan SQL ile global Order workflow geçiş seed’i yok (ilk kayıt şirket override olarak oluşur).
  - **Settings UI (Estimate):** `GET/PUT /settings/estimate-workflow` (permission: `settings.estimate_workflow.view|edit`). Same soft-delete + sync behavior as Order. İlk **PUT** sırasında şirket workflow tanımında henüz geçiş yoksa, global şablondaki geçişler şirket tanımına **klonlanır** (aksi halde silinen satır yalnızca global tarafta kalır ve “Show deleted” şirket kaydında görünmez). İzin backfill: **`DB/blinds-postgresql.sql`** (migration **44**). Varsayılan global geçiş seed’i yok (ilk kayıt Settings’ten şirket override olarak oluşur).
...
- **Status lifecycle (best practice):**
  ...
  - **Custom statuses** (`builtin_kind` null) may be deactivated **only when unused**:
    ...
    - Not referenced by domain data (`orders.status_order_id` (legacy: `status_orde_id`), `estimate.status_esti_id`).
  - **Order built-ins** use `status_order.builtin_kind` (şema: **`DB/blinds-postgresql.sql`**, migration **45**). Order/Estimate workflows resolve built-ins via `builtin_kind` (no hardcoded ids).
- **Ürün kategorisi şirket matrisi (`DB/31_company_blinds_product_category_matrix.sql`):** Global **`blinds_product_category`** ilk kurulumda **boş** (varsayılan Classic/Delux/Premium satırı yok); kategoriler **Lookups** üzerinden eklenir. **`company_blinds_product_category_matrix`**. **`GET /lookups/blinds-product-categories`**: isteğe bağlı **`catalog_scope`** — **`tenant`** (varsayılan; matriste etkin kategoriler, sipariş/ayar uçlarıyla uyumlu) veya **`global`** (tam katalog; yönetim ekranı için **`lookups.product_categories.edit`** veya **`lookups.edit`** gerekir). **`GET /orders/lookup/blinds-order-options`** ve **`GET /settings/blinds-category-matrix`** sütunları matriste etkin kodlarla sınırlıdır. Matris: **`GET`/`PUT /permissions/product-category-matrix`** (`lookups.product_categories.view|edit` veya geniş **`lookups.view`/`lookups.edit`**). Yeni şirket: matris ön dolumu yalnızca **`BOOTSTRAP_PREFILL_COMPANY_LOOKUP_MATRICES=true`** iken; yeni kategori oluşturulunca tüm şirketlere matrix satırı eklenir. MariaDB’de **`PUT`** “enable” yazımı **`INSERT IGNORE`** ile idempotent (**`(company_id, category_code)`** duplicate PRIMARY engeli).
- **Global blinds types (`DB/32_global_blinds_type_and_matrix.sql`):** `blinds_type` artık **şirketsiz** global katalog (`id` PK); **`company_blinds_type_matrix`** hangi şirketin hangi tipi kullanacağını tutar. **`GET /lookups/blinds-types`**: **`catalog_scope=tenant`** (varsayılan; aktif şirket matrisi) veya **`global`** (tam katalog; **`lookups.blinds_types.edit`** veya **`lookups.edit`**). **`GET`/`PUT /permissions/blinds-type-matrix`** — ürün kategorisi matrisiyle aynı hücre modeli (`MatrixPutIn`). Tahmin/sipariş/ayar uçları yalnızca matriste etkin tipleri listeler. Yeni şirket için tip matrisi ön dolumu yalnızca **`BOOTSTRAP_PREFILL_COMPANY_LOOKUP_MATRICES=true`** iken; yeni tip oluşturulunca tüm şirketlere matrix satırı eklenir. MariaDB’de **`PUT`** “enable” yazımı **`INSERT IGNORE`** ile idempotent (**`(company_id, blinds_type_id)`** duplicate PRIMARY engeli).
- **Blinds line extra matrices removed:** lifting/cassette option lists and **`blinds_line_extra_*`** / **`blinds_type_extra_allowed`** tables are dropped from the canonical schema (`DB/blinds-postgresql.sql`). Former APIs **`/lookups/blinds-extra-option-kinds`**, **`/lookups/blinds-extra-options/...`**, **`/settings/blinds-extra-matrix-kinds`**, **`/settings/blinds-extra-matrix/{kind_id}`** no longer exist. **`GET`/`PUT /settings/blinds-category-matrix`** remains the only blinds-line matrix under Settings (product category × blinds type).
