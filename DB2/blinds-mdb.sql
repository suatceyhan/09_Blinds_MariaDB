-- blinds-mdb.sql — MariaDB-compatible version of DB2/blinds-postgresql.sql
-- MariaDB 10.11+ recommended (12.2 OK).
-- Fresh DB: mysql -u USER -p < DB2/blinds-mdb.sql

CREATE DATABASE IF NOT EXISTS blinds CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE blinds;

SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Schema import (empty DB): allow DDL with circular / out-of-order FKs (mysqldump pattern).
-- Re-enabled at end of file. Application sessions still enforce FKs at runtime.
...
--   mysql -u <kullanıcı> -p <veritabanı> < DB2/blinds-mdb.sql
...
-- GENERATED disallowed many expressions on this engine (ERROR 1901): plain column + triggers maintain uniqueness key.
ALTER TABLE pending_employee_self_registrations
  ADD COLUMN IF NOT EXISTS email_unique_key VARCHAR(512) NULL;
DROP TRIGGER IF EXISTS tr_pending_employee_bi_email_unique_key;
DROP TRIGGER IF EXISTS tr_pending_employee_bu_email_unique_key;
DELIMITER $$
CREATE TRIGGER tr_pending_employee_bi_email_unique_key
BEFORE INSERT ON pending_employee_self_registrations FOR EACH ROW
BEGIN
  SET NEW.email_unique_key = IF(NEW.is_deleted, NULL, LOWER(NEW.email));
END$$
CREATE TRIGGER tr_pending_employee_bu_email_unique_key
BEFORE UPDATE ON pending_employee_self_registrations FOR EACH ROW
BEGIN
  SET NEW.email_unique_key = IF(NEW.is_deleted, NULL, LOWER(NEW.email));
END$$
DELIMITER ;
UPDATE pending_employee_self_registrations
SET email_unique_key = IF(is_deleted, NULL, LOWER(email))
WHERE email_unique_key IS NULL;
...
-- Uniqueness: same (company, normalized email) as PostgreSQL’s single generated concat column.
-- MariaDB: cannot reference FK/CASCADE columns (e.g. company_id) inside GENERATED — use composite UNIQUE instead.
ALTER TABLE customers
  ADD COLUMN IF NOT EXISTS email_normalized VARCHAR(512)
  GENERATED ALWAYS AS (NULLIF(LOWER(TRIM(email)), '')) VIRTUAL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_customers_company_email
  ON customers (company_id, email_normalized);
...
ALTER TABLE estimate DROP CONSTRAINT IF EXISTS ck_estimate_lead_source;
...
-- Prefix on TEXT lead_source: full-column composite index exceeds InnoDB max key length (utf8mb4).
CREATE INDEX IF NOT EXISTS idx_estimate_company_lead_source ON estimate (company_id, lead_source(32));
...
-- InnoDB composite FK on referencing columns requires a compatible index (left-prefix).
CREATE INDEX IF NOT EXISTS idx_orders_company_blinds_type_add_first
  ON orders (company_id, blinds_type_add_id);

-- PostgreSQL allows ON DELETE SET NULL for composite FKs (only nullable cols cleared).
-- MariaDB/InnoDB requires every FK column nullable for SET NULL — orders.company_id is NOT NULL,
-- so SET NULL is invalid here (ERROR 1825). Use RESTRICT; unlink via UPDATE blinds_type_add_id before DELETE parent.
...
CREATE INDEX IF NOT EXISTS idx_leads_company_status ON leads (company_id, status(64));
...
CREATE INDEX IF NOT EXISTS idx_estimate_company_status
  ON estimate (company_id, status(32));
...
-- Prefix on TEXT columns (entity_id can be long): keep composite index under InnoDB key length limit.
CREATE INDEX IF NOT EXISTS idx_attachments_company_entity ON attachments (company_id, entity_type(16), entity_id(191));
...
-- Composite FK + NOT NULL company_id: MariaDB rejects ON DELETE SET NULL (ERROR 1825); use RESTRICT.
...
-- MariaDB: use JOIN (not PostgreSQL UPDATE ... FROM).
UPDATE estimate_blinds eb
INNER JOIN estimate e
  ON e.company_id = eb.company_id
  AND e.id = eb.estimate_id
SET eb.perde_sayisi = e.perde_sayisi
WHERE eb.perde_sayisi IS NULL
...
-- Full-column index (PostgreSQL partial index WHERE … not portable before MariaDB partial indexes).
CREATE INDEX IF NOT EXISTS idx_blinds_product_category_active
  ON blinds_product_category (active);
...
CREATE INDEX IF NOT EXISTS idx_status_estimate_company_active
  ON status_estimate (company_id, active);
...
ALTER TABLE status_estimate DROP CONSTRAINT IF EXISTS ck_status_estimate_slug;
-- Both names: CREATE TABLE UNIQUE + migration 20 NN index (PostgreSQL partial parity); all must go before DROP COLUMN slug.
DROP INDEX IF EXISTS uq_status_estimate_company_slug_nn ON status_estimate;
DROP INDEX IF EXISTS uq_status_estimate_company_slug ON status_estimate;
-- Idempotent DROP COLUMN (avoids IF EXISTS + InnoDB edge cases with leftover keys on MariaDB < 10.11).
SET @mdb_drop_slug := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
   WHERE TABLE_SCHEMA = DATABASE() AND TABLE_SCHEMA <> ''
     AND TABLE_NAME = 'status_estimate' AND COLUMN_NAME = 'slug') > 0,
  'ALTER TABLE status_estimate DROP COLUMN slug',
  'SELECT 1 WHERE 0'
);
PREPARE mdb_stmt_drop_slug FROM @mdb_drop_slug;
EXECUTE mdb_stmt_drop_slug;
DEALLOCATE PREPARE mdb_stmt_drop_slug;
...
BEGIN;
-- ---- Drop dependent FKs ----
-- InnoDB: FOREIGN KEY must be dropped with DROP FOREIGN KEY (DROP CONSTRAINT alone may leave the same identifier).
ALTER TABLE estimate DROP FOREIGN KEY IF EXISTS fk_estimate_status_estimate;
ALTER TABLE estimate DROP CONSTRAINT IF EXISTS fk_estimate_status_estimate;
...
SELECT SUBSTRING(MD5(CAST(UUID() AS CHAR)), 1, 16), 'New Estimate', TRUE, -1, 'new'
...
SELECT SUBSTRING(MD5(CAST(UUID() AS CHAR)), 1, 16), 'Pending', TRUE, 0, 'pending'
WHERE NOT EXISTS (SELECT 1 FROM status_estimate se WHERE se.builtin_kind = 'pending');
INSERT INTO status_estimate (id, name, active, sort_order, builtin_kind)
SELECT SUBSTRING(MD5(CAST(UUID() AS CHAR)), 1, 16), 'Converted to order', TRUE, 1, 'converted'
WHERE NOT EXISTS (SELECT 1 FROM status_estimate se WHERE se.builtin_kind = 'converted');
INSERT INTO status_estimate (id, name, active, sort_order, builtin_kind)
SELECT SUBSTRING(MD5(CAST(UUID() AS CHAR)), 1, 16), 'Cancelled', TRUE, 2, 'cancelled'
...
SELECT SUBSTRING(MD5(CAST(UUID() AS CHAR)), 1, 16), 'New order', TRUE, 0, 'new'
...
CREATE TEMPORARY TABLE tmp_est_map AS
SELECT
  l.company_id,
  l.id AS old_id,
  COALESCE(
...
GROUP BY lower(trim(name));
CREATE TEMPORARY TABLE tmp_ord_map AS
SELECT
  l.company_id,
  l.id AS old_id,
  CASE
...
FROM companies c
WHERE c.is_deleted IS NOT TRUE;
-- Drop FKs that reference legacy parents *before* DROP legacy tables — otherwise InnoDB can drop the
-- FK implicitly but leave the index name `fk_estimate_status_estimate`, causing ERROR 1061 on ADD.
ALTER TABLE estimate DROP FOREIGN KEY IF EXISTS fk_estimate_status_estimate;
ALTER TABLE estimate DROP CONSTRAINT IF EXISTS fk_estimate_status_estimate;
ALTER TABLE orders DROP FOREIGN KEY IF EXISTS fk_orders_status_order;
ALTER TABLE orders DROP CONSTRAINT IF EXISTS fk_orders_status_order;
-- ---- Drop legacy ----
DROP TABLE IF EXISTS status_estimate_legacy;
DROP TABLE IF EXISTS status_order_legacy;
-- ---- FKs (single-column, global catalogs) ----
DROP INDEX IF EXISTS fk_estimate_status_estimate ON estimate;
DROP INDEX IF EXISTS fk_orders_status_order ON orders;
ALTER TABLE estimate
  ADD CONSTRAINT fk_estimate_status_estimate
...
BEGIN;
-- FK blocks MODIFY; column stays VARCHAR(16) to match customers.id (PostgreSQL: ALTER COLUMN … DROP NOT NULL only).
ALTER TABLE estimate DROP FOREIGN KEY IF EXISTS fk_estimate_customer;
ALTER TABLE estimate MODIFY COLUMN customer_id VARCHAR(16) NULL;
ALTER TABLE estimate
...
-- (copy kept in DB2/32_global_blinds_type_and_matrix.sql for standalone runs.)
...
DROP TABLE IF EXISTS blinds_type_legacy;

ALTER TABLE estimate_blinds DROP FOREIGN KEY IF EXISTS fk_estimate_blinds_blinds_type;
ALTER TABLE estimate_blinds DROP CONSTRAINT IF EXISTS fk_estimate_blinds_blinds_type;
DROP INDEX IF EXISTS fk_estimate_blinds_blinds_type ON estimate_blinds;
...
ALTER TABLE estimate DROP FOREIGN KEY IF EXISTS fk_estimate_blinds_type;
ALTER TABLE estimate DROP CONSTRAINT IF EXISTS fk_estimate_blinds_type;
DROP INDEX IF EXISTS fk_estimate_blinds_type ON estimate;
ALTER TABLE estimate
...
ALTER TABLE blinds_type_add DROP FOREIGN KEY IF EXISTS fk_blinds_type_add_blinds_type;
ALTER TABLE blinds_type_add DROP CONSTRAINT IF EXISTS fk_blinds_type_add_blinds_type;
DROP INDEX IF EXISTS fk_blinds_type_add_blinds_type ON blinds_type_add;
...
ALTER TABLE blinds_type_category_allowed DROP FOREIGN KEY IF EXISTS fk_btca_blinds_type;
ALTER TABLE blinds_type_category_allowed DROP CONSTRAINT IF EXISTS fk_btca_blinds_type;
DROP INDEX IF EXISTS fk_btca_blinds_type ON blinds_type_category_allowed;
...
-- • Bu dosya DB2/blinds-postgresql.sql içeriğinin MariaDB uyarlamasıdır.
...
-- PostgreSQL: UNIQUE on COALESCE(company_id, nil-uuid). This MariaDB rejects IFNULL/CASE in GENERATED (ERROR 1901): column + triggers.
ALTER TABLE workflow_definitions
  ADD COLUMN IF NOT EXISTS company_id_norm CHAR(36) NULL;
DROP TRIGGER IF EXISTS tr_workflow_definitions_bi_company_id_norm;
DROP TRIGGER IF EXISTS tr_workflow_definitions_bu_company_id_norm;
DELIMITER $$
CREATE TRIGGER tr_workflow_definitions_bi_company_id_norm
BEFORE INSERT ON workflow_definitions FOR EACH ROW
BEGIN
  SET NEW.company_id_norm = IFNULL(NEW.company_id, '00000000-0000-0000-0000-000000000000');
END$$
CREATE TRIGGER tr_workflow_definitions_bu_company_id_norm
...
SET company_id_norm = IFNULL(company_id, '00000000-0000-0000-0000-000000000000')
WHERE company_id_norm IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_workflow_definitions_company_entity_code_version
  ON workflow_definitions (company_id_norm, entity_type(191), code(191), version);
...
FOR EACH ROW
BEGIN
  -- Global catalog `status_order` has no company_id (migration 27); orders reference status by global id only.
  UPDATE orders
  SET status_orde_id = NULL
  WHERE status_orde_id = OLD.id;
END$$
DELIMITER ;
